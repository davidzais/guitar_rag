...
export function MessageList({ messages, isPending, error }: Props) {
  ...
  useEffect(() => {
    if (!isPending) return;
    const id = setTimeout(() => setSlowTimer(true), 5000)
    return () => {
      clearTimeout(id)
      setSlowTimer(false)
    }  // cleanup: runs on isPending change OR unmount
  }, [isPending])
  ...
  const errorMessage =
    error instanceof ApiError && error.status === 429
      ...
      : error instanceof ApiError && error.status === 408
        ? error.message            // the friendly timeout text from api.ts
        : error
      ...
      {isPending && (
        <div className="flex justify-start gap-3">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl rounded-tl-sm px-4 py-3">
            ...
            {slowTimer && (
              <div>Waking up the tutor… first question after a break takes a moment 🎸</div>
            )}
          </div>
        </div>
      )}
